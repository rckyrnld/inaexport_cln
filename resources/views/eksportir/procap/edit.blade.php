...
@section('content')
                ...
                <div class="card">
                    @foreach ($data as $val)
                        <div class="card-header border-bottom">
                            <h3 class="mb-0">Edit Kapasitas Produksi</h3>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="form-row">
                                <div class="form-group col-sm-3 sm-2">
                                    ...
                                    <select class="form-control" name="tahun" id="tahun">
                                        ...
                                        @foreach ($years as $sa)
                                        <option value="{{ $sa }}" @if($sa == $val->tahun) selected @endif>{{ $sa }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-row">
                                ...
                                <div class="form-group col-sm-3 sm-2">
                                    ...
                                    <input onkeyup="sum();" type="text" class="form-control amount" value="{{ str_replace(".", ",", $val->sendiri_persen ) }}"
                            ...
                            <div class="form-row">
                                <div class="form-group col-sm-3 sm-2">
                                    ...
                                    <input type="text" class="form-control amount" value="{{ str_replace(".", ",", $val->outsourcing_persen )}}"
                                        name="out_persen" id="out_persen" readonly>
                            ...
                            <div class="form-row rightbtn">
                                <div class="form-group col-sm-12">
                                    ...
                                    <button class="btn btn-primary" id="btnSubmit" type="button">Update
                                    </button>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        function sum() {
            var txtFirstNumberValue = $('#persen_sendiri').val().replace(',','.');
            var result = 100 - txtFirstNumberValue;
            console.log(result);
            if (!isNaN(result)) {
                $('#out_persen').val(result.toString().replace('.', ','));
            }
        }
        
        
        $("#persen_sendiri").keyup(function(){
            var domestic = $('#persen_sendiri').val().replace(',','.');
            var max = 100;
            if(domestic > max){
                $('#persen_sendiri').addClass('input-error');
                $("#btnSubmit").attr("type","button");
                $("#btnSubmit").attr("onclick","notifAlert()");
            }else{
                $('#persen_sendiri').removeClass('input-error');
        ...
        function notifAlert(){
            alert('Own Production Melebihi 100');
        }
