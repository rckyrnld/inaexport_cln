...
@section('content')
    ...
    <div class="row">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form id="form_video_conference" class="form-horizontal" enctype="multipart/form-data" method="POST"
                        ...
                        {{ csrf_field() }}
                        <div class="row">
                            <input type="hidden" name="id_busnisnya" id="id_busnisnya"
                                value="{{ $id != '' ? $data->id : '' }}">
                            <div class="col-lg-6">
                                ...
                                <div class="form-group row mb-2 ml-2">
                                    ...
                                    <div class="col-sm-9">
                                        ...
                                        <input type="hidden" id="hidden_start_time"
                                            value="{{ $data != '' ? date('d-m-Y H:i', strtotime($data->start_time)) : '' }}">
                                    </div>
                                </div>
                                <div class="form-group row mb-2 ml-2">
                                ...
                                @if (Auth::user() != '')
                                    <div class="form-group row mb-2 ml-2">
                                        ...
                                        <div class="col-sm-9">
                                            <select name="states_buyer" id="states_buyer" class="form-control select2"
                                                style="width: 100%">
                                                <option value=""></option>
                                                @if ($id != '')
                                                    @foreach ($data_buyer as $db)
                                                        <option value="{{ $db->id }}" @if (isset($data))
                                                            @foreach ($data->video_conference_participants as $vcp)
                                                                @if ($vcp->itdp_company_user_id == $db->id) selected @endif
                                                            @endforeach
                                                    @endif>{{ $db->profile_buyer->company }}
                                                    </option>
                                                @endforeach
                                            @else
                                                @foreach ($data_buyer as $db)
                                                    <option value="{{ $db->id }}">
                                                        {{ $db->profile_buyer->company }}</option>
                                                @endforeach
                                @endif
                                </select>
                            </div>
                        ...
                        <div class="form-group row mb-2 ml-2">
                            ...
                            <div class="col-sm-9">
                                <select name="states_exportir" id="states_exportir" class="form-control select2"
                                    style="width: 100%">
                                    @if ($id != '')
                                        @foreach ($data_eks as $de)
                                            <option value="{{ $de->id }}" @if (isset($data))
                                                @foreach ($data->video_conference_participants as $vcp)
                                                    @if ($vcp->itdp_company_user_id == $de->id) selected @endif
                                                @endforeach
                                        @endif>
                                        {{ $de->profile != '' ? $de->profile->company : '' }}</option>
                                    @endforeach
                                @else
                                    @foreach ($data_eks as $de)
                                        <option value="{{ $de->id }}">
                                            {{ $de->profile != '' ? $de->profile->company : '' }}
                                        </option>
                                    @endforeach
                                    @endif
                                </select>
        ...
        @else
            <div class="form-group row mb-2 ml-2">
                ...
                <label for="myInviteExportir" class="col-sm-3 col-form-label font-weight-bold">Supplier</label>
                <div class="col-sm-9">
                    <select name="states_exportir" id="states_exportir" class="form-control select2" style="width: 100%">
                        @if ($id != '')
                            @foreach ($data_eks as $de)
                                <option value="{{ $de->id }}" @if (isset($data))
                                    @foreach ($data->video_conference_participants as $vcp)
                    ...
                    @else
                        @foreach ($data_eks as $de)
                            <option value="{{ $de->id }}">{{ $de->profile != '' ? $de->profile->company : '' }}
                            </option>
                        @endforeach
                        @endif
                    </select>
            ...
            <div class="form-group row mb-2 ml-2">
                ...
                <div class="col-sm-9">
                    <select name="states_perwakilan" id="states_perwakilan" class="form-control select2"
                        style="width: 100%">
                        <option value=""></option>
                        @if ($id != '')
                            @foreach ($data_perwakilan as $db)
                                <option value="{{ $db->id }}" @if (isset($data))
                                    @foreach ($data->video_conference_participants as $vcp)
                                        @if ($vcp->itdp_admin_user_id == $db->id) selected @endif
                                    @endforeach
                            @endif>{{ $db->name }}</option>
                        @endforeach
                    @else
                        @foreach ($data_perwakilan as $dp)
    ...
    <script>
        $(function() {
            ...
            $('#inputStartTime').val($('#hidden_start_time').val());
            // Select2
