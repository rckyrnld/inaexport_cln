...
class ManagementNoAuthController extends Controller
{
    ...
    public function RegisterExp(Request $request)
    {
        ...
        $check_email_exist = DB::table('itdp_company_users')->where('email', $email)->first();
        $exist = false;
    ...
    {
        $research = EventDetail::with('contact_person', 'country_name', 'participants')
        ...
        if (count($research) > 0) {
            ...
            foreach ($research as $item) {
                ...
                array_push($getJSON, array(
                    ...
                    "supplier_count" => count($item->participants),
                    "event_place" => $item->country_name->country,
                    "label" => $register_open,
                    "btn_show" => $btn_show
                    // "supplier_count" => DB::table('event_interest')->where('id_event', $item->id)->count('id'),
                    //                    "type" => $item->type,
    ...
    public function getProvinceFilter(Request $req)
    {
        $query = DB::table('mst_province')->orderby('province_en', 'asc');
        if ($req->filter != null) {
            $query->where('province_en', 'ILIKE', '%' . $req->filter . '%');
        }
        $province = $query->get();
    ...
    public function getSub(Request $request)
    {
        ...
        if ($level == 1) {
            ...
            $catprod = DB::table('csc_product')->where('level_1', $request->idparent)->where('level_2', $request->idsub)->orderBy('nama_kategori_en', 'ASC')->get();
            if (count($catprod) > 0) {
                $meta = [
                    'code' => '200',
                    'message' => 'Success',
