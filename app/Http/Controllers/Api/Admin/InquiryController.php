...
//            ->first()->username;
    ...
    public function list_inquiry_perwadag(Request $request)
    {
        $data=[];
        $dataSeries=[];
        $id_user = $request->id_user;

        $buying = DB::select("select ROW_NUMBER() OVER (ORDER BY id DESC) AS Row, * from csc_buying_request where id_pembuat='" . $id_user . "' and  by_role='4' and deleted_at ISNULL order by date desc ");
    ...
    public function inquiry_detail_perwadag()
    {
        $data=[];
        $id_inquiry = ($_GET['id_inquiry']) ? $_GET['id_inquiry'] : NULL;
        $data['detail'] = DB::table('csc_buying_request')->where('id',$id_inquiry)->first();
    ...
    public function inquiry_detail_perwadag_new(){
        $id_inquiry = ($_GET['id_inquiry']) ? $_GET['id_inquiry'] : NULL;
        $data['detail'] = DB::table('csc_buying_request')->where('id',$id_inquiry)->first();
        
        $ex = explode(',',$data['detail']->id_csc_prod);
        $kat = DB::table('csc_product')->where('id', (int)$ex[0])->first();

        $id_country_BR = explode(',',$data['detail']->id_mst_country);
        $country = DB::table('mst_country')->where('id', (int)$id_country_BR[0])->first();

        //dd($kat->nama_kategori_en);
        $data['detail']->kat = $kat->nama_kategori_en;
        $kat = DB::table('csc_product')->where('id', (int)$ex[1])->first();
        $data['detail']->kat1 = $kat ? $kat->nama_kategori_en : null;
        $kat = DB::table('csc_product')->where('id', (int)$ex[2])->first();
        $data['detail']->kat2 = $kat ? $kat->nama_kategori_en : null;

        $data['detail']->country_name = $country ? $country->country : null;
        ...
        //DB::table('id_csc_prod')->when
    }

    
    public function br_importir_chat(Request $request)
    {
        $data=[];
        $dataSeries=[];
        ...
        $chat = DB::select("select * from csc_buying_request_chat where id_br='".$id_br."' and id_join='".$id_join."' order by tanggal ASC");
        $join = DB::table('csc_buying_request_join')->where('id',$id_join)->where('id_br', $id_br)->get();
        $buying_requests = DB::table('csc_buying_request')->where('id',$id_br)->get();

        
        $data['chat'] = $chat;
        $data['join'] = $join;
        $data['buying_requests'] = $buying_requests;
