...
class BuyingreqController extends Controller
{
    ...
    public function br_importir_save_new(Request $req)
    {
        DB::beginTransaction();

        try {
            $validator =  Validator::make($req->all(), [
                'profeks' => 'required|integer',
                'subyek' => 'max:255|nullable',
                'valid' => 'integer|nullable',
                'country' => 'integer|nullable',
                'city' => 'max:255|nullable',
                'cat' => 'integer|nullable',
                'cat1' => 'integer|nullable',
                'cat2' => 'integer|nullable',
                'eo' => 'max:255|nullable',
                'neo' => 'max:255|nullable',
                'tp' => 'numeric|nullable',
                'id_user' => 'integer|nullable',
                'idprod' => 'integer|nullable',
            ]);
            ...
            $ekss = (int)$req->profeks;
            $eks = DB::table('itdp_company_users')->where('id_profil', $ekss)->first();
            ...
            $data['insert'] = BuyingRequest::create([
                'subyek' => $req->subyek,
                'valid' => $req->valid,
                'id_mst_country' => $req->country,
                'city' => $req->city,
                'id_csc_prod_cat' => $req->cat,
                'id_csc_prod_cat_level1' => $req->cat1,
                'id_csc_prod_cat_level2' => $req->cat2,
                'shipping' => $req->ship,
                'spec' => $req->spec,
                'eo' => $req->eo,
                'neo' => $req->neo,
                ...
                'ntp' => $req->ntp,
                'by_role' => '3',
                'id_pembuat' => $req->id_user,
                'date' => date('Y-m-d H:m:s'),
                'id_csc_prod' => $req->kumpulcat,
                'publish' => ($req->publish == "true") ? 'true' : 'false',
                'id_csc_product_single' => $req->idprod
            ]);


            $data['insert_br'] = DB::table('csc_buying_request_join')->insert([
                'id_br' => $data['insert']->id,
                'id_eks' => $eks->id,
                'status_join' => 0,
                'date' => date('Y-m-d')
            ]);
 ...
 */
    ...
    public function data_br_new(Request $req)
    {
        $data = [];
        $dataSeries = [];
        $loc = app()->getLocale();
        ...
        // $buying = buying_and_inqueri::where('id_pembuat', $id_user)->orderBy('date', 'DESC')->get();
        $buying = DB::table('csc_buying_request')->where('id_pembuat', $id_user)->orderBy('date', 'DESC')->get();

        $buying = $buying->map(function ($item) {
            $id_csc = explode(",", $item->id_csc_prod);
            $list_k = array();
            for ($a = 0; $a < count($id_csc); $a++) {
                if (!empty($id_csc[$a]) || $id_csc[$a] != null) {
                    $list_k[] = $id_csc[$a];
            ...
            $item->kategori_desc_0 = isset($getName[0]) ? $getName[0]->nama_kategori_en : '';
            $item->kategori_desc_1 = isset($getName[1]) ? $getName[1]->nama_kategori_en : '';
            $item->kategori_desc_2 = isset($getName[2]) ? $getName[2]->nama_kategori_en : '';

            return $item;
        });

        foreach ($buying as $buy) {
            $dataSeries1 = DB::table('itdp_company_users as users')
                ->select('users.*', 'join.*', 'eks.company')
                ->join('csc_buying_request_join as join', 'join.id_eks', 'users.id')
                ->join('itdp_profil_eks as eks', 'eks.id', 'users.id_profil')
                ->where('join.id_br', $buy->id)
                ->orderBy('date', 'DESC')
                ->get();
            foreach ($dataSeries1 as $b) {
                $dataSeries[] = DB::table('itdp_company_users as users')
                    ->select('users.*', 'join.*', 'eks.company')
                    ->join('csc_buying_request_join as join', 'join.id_eks', 'users.id')
                    ->join('itdp_profil_eks as eks', 'eks.id', 'users.id_profil')
                    ->where('join.id', $b->id)
        ...
        $data['buying'] = $buying;
        $data['buying_requests'] = $dataSeries;
    ...
    public function data_br_published(Request $req)
    {
        $data = [];
        $dataSeries = [];
        $loc = app()->getLocale();
        ...
        $buying =   BuyingRequest::select(DB::raw('distinct on
            ...
            ->orderBy('csc_buying_request.date', 'desc')
            ->paginate(10);
    ...
    public function data_br_published_all(Request $req)
    {
        $data = [];
        $dataSeries = [];
        $loc = app()->getLocale();
    ...
    public function eksfrontBR(Request $r)
    {

        // dd($request->all());
        /* $cr = explode(',', $r->id_laporan);
		$hitung = count($cr); */
        // $category = $cr[$hitung - 2];
        $category = $r->category;
        $sub_category_1 = $r->sub_category_1;
        $sub_category_2 = $r->sub_category_2;
        // $no = 1;
    ...
    public function savefrontBR(Request $req)
    {
        //dd($req);
        $data['insert'] = DB::select("
                insert into csc_buying_request(
                    ...
                    '" . date('Y-m-d H:m:s') . "',
                    '" . $req->kumpulcat . "',
                    '" . (($req->publish == "true") ? 'true' : 'false') . "')
                    RETURNING id");

        $eks = $req->seleks;
        $ek = ($eks != '[]') ? explode(',', str_replace(']', '', str_replace('[', '', $eks))) : [];
        // dd(count($ek));
        //dd($req->seleks);
        for ($i = 0; $i < count($ek); $i++) {
            $data['insert_br'] = DB::table('csc_buying_request_join')->insert([
                'id_br' => $data['insert'][0]->id,
                'id_eks' => (int)$ek[$i],
                'status_join' => 0,
                'date' => date('Y-m-d')
            ]);
    ...
    public function ekslistbr(Request $request)
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            for ($k = 0; $k < 3; $k++) {
                $jsonResult[$i]["kategori_desc_$k"] = isset($getName[$k]) ? $getName[$k]->nama_kategori_en : '';
            }
    ...
    public function br_save_join(Request $request)
    {
        ...
        $id_br = DB::table('csc_buying_request_join')
            ->where('id', $id)
            ->first()->id_br;
        $update_csc_br = DB::table('csc_buying_request')
            ->where('id', $id_br)
            ->update([
                'status' => '1'
            ]);

        if ($update && $update_csc_br) {
    ...
    public function simpanchatbr(Request $request)
    {
		...
		");
            ...
            //     'id' => $id6
            // ];
            // if (env('MAIL_USERNAME', 'no-reply@inaexport.id') != '') {
            //     Mail::send('UM.user.sendbrchateks3', $data33, function ($mail) use ($data33) {
            //         $mail->to($data33['email1'], $data33['username']);
            //         $mail->subject('Importir Respond Chat On Buying Request');
            //     });
