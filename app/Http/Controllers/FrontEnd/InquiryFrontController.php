...
class InquiryFrontController extends Controller
{
    ...
    public function create($id)
    {
        if(Auth::guard('eksmp')->user() != null){
            if(Auth::guard('eksmp')->user()->id_role == 3) {
                ...
                $pageTitle = "Create Inquiry | Inaexport";
                $topMenu = "";

                return view('frontend.inquiry.create_new', compact('country','profile','data', 'url', 'id_user', 'coinquiry', 'pageTitle', 'topMenu'));
            }else {
                return redirect('/');
            }
            ...
            if(Auth::user()->id_group == 4) {
                ...
                $country = DB::table('mst_country')->orderby('country','asc')->get();
                $profile = DB::table('itdp_admin_users')
                    ->select('country AS id_mst_country')
                    ->join('itdp_admin_ln', 'itdp_admin_ln.id', '=', 'itdp_admin_users.id_admin_ln')
                    ->where('itdp_admin_users.id', $id_user)
                    ->first();
    ...
    public function store($id, Request $request)
    {
        ...
        if(Auth::user() != null || Auth::guard('eksmp')->user() != null){
            if(Auth::user() != null) {
                $id_user = (Auth::user()->id_group == 4) ? Auth::user()->id : 0;
                $role = 4;
            } else if(Auth::guard('eksmp')->user() != null) {
                $id_user = (Auth::guard('eksmp')->user()->id_role == 3) ? Auth::guard('eksmp')->user()->id : 0;
                $role = 3;
            }
            ...
            // $save = DB::table('csc_buying_request')->insertGetId([
            //     'id_pembuat' => $id_user,
            //     'type' => $type,
            ...
            $insert = DB::select("insert into csc_buying_request
            (subyek,
            valid,
            id_mst_country,
            city,
            ...
            '".$request->country."',
            '".$request->city."',
            '".$dtproduct->id_csc_product."',
            '".(($dtproduct->id_csc_product_level1 > 0 ) ? $dtproduct->id_csc_product_level1 : 0)."',
            '".(($dtproduct->id_csc_product_level2 > 0 ) ? $dtproduct->id_csc_product_level2 : 0)."',
            '".$request->ship."',
            '".$request->spec."',
            '".$file."',
            '".$request->eo."',
            ...
            '".Date('Y-m-d H:m:s')."',
            '".$request->kumpulcat2."',
            '".(($request->publish == "true") ? 'true' : 'false')."') RETURNING id");

            $nama_file1 = NULL;
            $destination= 'uploads\Inquiry\\'.$insert[0]->id;
            ...
            $savefile = DB::table('csc_inquiry_br')->where('id', $insert[0]->id)->update([
                'file' => $nama_file1,
            ]);
            ...
            if($insert[0]->id){
                ...
                $insert_br = DB::table('csc_buying_request_join')->insert([
                    'id_br' => $insert[0]->id,
                    'id_eks' => $dtproduct->id_itdp_company_user,
                    'status_join' => 0,
                    'date' => date('Y-m-d')
                ]);
...
//                });
    ...
    public function chatting($id)
    {
        if(Auth::guard('eksmp')->user()->id_role == 3){
            ...
            $pageTitle = "Message | Inaexport";
            $topMenu = "";

            return view('frontend.inquiry.chatting', compact('inquiry', 'data', 'messages', 'id_user', 'pageTitle', 'topMenu'));
    ...
    public function view($id)
    {
        if(Auth::guard('eksmp')->user()->id_role == 3){
            ...
            $pageTitle = "View Inquiry | Inaexport";
            $topMenu = "";
 
            return view('frontend.inquiry.view', compact('inquiry', 'data', 'id_user', 'pageTitle', 'topMenu'));
        }else{
            return redirect('/');
        }
    }
